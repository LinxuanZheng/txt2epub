...
func GenBook() {
	...
	fileName, err := GetTxtFile()
	if err != nil {
		panic(err)
	} else if fileName == "" {
		panic("file not existed")
	}
	...
	book := NewBook(file)
	err = book.LoadBook()
	book.Config.UUID = uuid.New().String()
	if err != nil {
		fmt.Println(err)
	}
...
func GenDefaultCover() {
	workDir, err := os.Getwd()
	if err != nil {
		panic(err)
	}

	output, err := base64.StdEncoding.DecodeString(DefaultCover)
	if err != nil {
		panic(err)
	}

	err = ioutil.WriteFile(workDir+string(os.PathSeparator)+"cover.png", output, 0444)
	if err != nil {
		panic(err)
	}
}
